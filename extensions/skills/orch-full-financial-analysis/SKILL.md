...
用 **a-share-financial-forensic** 技能基于步骤 3 切割后的章节文件执行完整的财务排雷分析。每次均需执行（分析基于最新数据）。报告保存路径为： `data/reports/{code}/4_output/{股票代码}_{年份或年份范围}_财务分析报告.md`
...
1. 用 run_command 从切割文件中提取三张核心报表的关键数据（资产负债表、利润表、现金流量表）
