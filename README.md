...
- **Skill 插件扩展** — 通过 `SKILL.md` 声明式添加专业分析能力（财务造假识别、PDF 转 Markdown 等）
...
| `slow_think_interval` | 每 N 步触发一次全局复盘 | 3 |
| `token_budget` | Token 预算上限 | 100000 |
| `loop_detection_threshold` | 连续同工具调用次数上限 | 3 |

### 上下文压缩

长对话自动压缩以节省 Token：保留最近 20 条消息原文，旧消息截断至 2000 字符，总预算 100000 字符。可在设置页调整。
...
│   ├── tools/               # 数据获取工具（AKShare）
│   ├── skills/              # Skill 插件加载框架
│   └── context/             # 上下文压缩与运行时截断
├── extensions/skills/       # Skill 定义（8 个）
...
系统设置
  ...
  PUT    /api/settings/models                → 管理模型配置（CRUD）
...
在 `agent/tools/` 下继承 `BaseTool` 实现 `run()` 方法，工具会在启动时自动注册。
