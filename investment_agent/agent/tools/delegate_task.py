...
class DelegateTaskTool(BaseTool):
    ...
    description = (
        ...
        "完成后仅返回文本执行结果。适用于需要多步骤执行、工具调用密集的独立子任务，"
        "如下载财报、PDF转换、文件切割、专项分析等。支持任意已注册的非编排技能委派，"
        "支持嵌套委派（最大深度可配置）。\n\n"
        "【何时主动委派】分析当前任务结构，满足以下任一条件时应考虑使用本工具：\n"
        "（1）存在多个互不依赖的子任务 — 可并行委派以加速执行（如同时分析资产负债表、利润表、现金流量表）；\n"
        ...
        "【约束】不可委派的场景：子任务之间有严格的串行依赖、需要父Agent即时决策的交互式任务、"
