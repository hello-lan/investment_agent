...
class BaseLoopEngine:
    """执行循环基类：提供公共运行时能力与模板方法。"""
    ...
    def __init__(
        ...
        self.loop_state = None  # 预留给不同 loop 子类维护私有状态
    ...
    async def before_step(self, messages: list[dict], step: int) -> AsyncGenerator:
        """每步开始前的 hook。默认无操作。"""
    ...
    async def call_model(self, messages: list[dict], step: int) -> AsyncGenerator:
        """调用模型。默认复用公共 LLM 调用逻辑。"""
    ...
    def on_tool_calls_received(self, tool_calls: list[ToolCall]) -> None:
        """接收到 tool_calls 后的 hook。默认无操作。"""

    def on_tool_results(self, tool_calls: list[ToolCall], has_error: bool) -> None:
        """工具执行完成后的 hook。默认无操作。"""

    async def execute_actions(self, tool_calls: list[ToolCall]) -> AsyncGenerator:
        """执行动作层。当前默认动作为工具调用。"""
        tool_results = []
        has_error = False
        async for event in self._tool_executor.execute(tool_calls, self):
            if isinstance(event, _Value):
                ...
                if event.get("type") == EventType.TOOL_RESULT:
                    ...
                    if "error" in output.lower() or "失败" in output or "错误" in output:
                        has_error = True
                yield event
        yield _Value((tool_results, has_error))

    async def after_actions(
        self,
        messages: list[dict],
        ...
        """动作执行后的 hook。默认记录结果并回灌上下文。"""
        self.on_tool_results(tool_calls, has_error)
        messages.append({"role": "user", "content": tool_results})
        yield _Value(messages)

    def build_done_event(self) -> dict:
        """构造 done 事件。"""
        return {
            "type": EventType.DONE,
            "usage": {
    ...
    async def run(self, messages: list[dict]) -> AsyncGenerator[dict, None]:
        ...
        messages = self.prepare_run(messages)
        ...
        while step < self.max_steps:
            ...
            async for event in self.before_step(messages, step):
                if isinstance(event, _Value):
            ...
            async for event in self.maybe_pre_llm_phase(messages, step):
                if isinstance(event, _Value):
                    messages = event.value
                else:
                    yield event
            ...
            response = None
            async for event in self.call_model(messages, step):
                if isinstance(event, _Value):
            ...
            async for event in self.after_llm_response(messages, response, step):
                if isinstance(event, _Value):
            ...
            if has_tool_calls:
                self.on_tool_calls_received(response.tool_calls)
                ...
                tool_results = []
                has_error = False
                async for event in self.execute_actions(response.tool_calls):
                    if isinstance(event, _Value):
                        tool_results, has_error = event.value
                    else:
                        yield event

                async for event in self.after_actions(
                    messages, tool_results, response.tool_calls, step, has_error,
                ):
                    if isinstance(event, _Value):
                        messages = event.value
                    else:
    ...
    def _inject_date(messages: list[dict]) -> None:
        ...
        if isinstance(content, str):
            ...
            if content and isinstance(content[0], dict) and content[0].get("type") == "text":
                ...
                first["text"] = date_note + str(first.get("text", ""))
                msg["content"] = [first] + content[1:]
            else:
    ...
    async def _process_response(
        ...
        if not response.tool_calls:
            ...
            yield _Terminal(self.build_done_event())
            return
    ...
    def _extract_role_from_system(self) -> str:
        """从 system_prompt 中提取角色定义（第一段），用于精简 system。"""
